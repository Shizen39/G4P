...
def crossover(a,b,elite,fitness,n_chromosomes,select_probs):
    ...
    print('~ crossing over ', np.cbrt(fitness[a]),' and ', np.cbrt(fitness[b]))
    policy_A = elite[a]
    policy_B = elite[b]
    child_policy1 = dict()
    child_policy2 = dict()
...
def prnt_genes(chromosome):
    return ''.join(str(e) for e in chromosome.values())
...
def evolve(env_id, n_chromosomes, n_generations, bins, sid=1234, n_episodes=250, mut_prob=0.05, max_elite=5): #20 50 5 250
    ...
    for generation in range(n_generations): #meiosi
        ...
        #-------EVALUATE MODELS--------#
        pop_scores = run_parallel_eval(env_id, sid, pool, n_episodes, population)   #pop_scores = [evaluate_policy(env, p, n_episodes, i) for i,p in enumerate(population)]
        pop_fitness = np.mean(pop_scores, axis=1) #mean of all episode_rewards for each policy

        elite_threashold = np.mean(pop_fitness)
        elite_threashold = np.power(elite_threashold,3)

        pop_fitness = np.power(pop_fitness, 3)
        # print('\n ****** Generation', generation+1, 'max score = ', max(pop_fitness), ' elite_threashold = ',elite_threashold,' ******\n')
        print('\n ****** Generation', generation+1, 'max score = ', np.cbrt(max(pop_fitness)), ' elite_threashold = ',np.cbrt(elite_threashold),' ******\n')
        ...
        #survive only those fitness is greater then  mean of all fitness
        elite = [e for i,e in enumerate(population) if pop_fitness[i]>=elite_threashold] #pick firsts 5 best policies
        elite_score = [e for i,e in enumerate(pop_scores) if pop_fitness[i]>=elite_threashold]#[evaluate_policy(env, p, n_episodes, prnt=False) for p in elite]
        elite_fitness = list(np.mean(elite_score, axis=1))
        elite_fitness = list(np.power(elite_fitness, 3))
        ...
        if np.sum(elite_fitness) <0:
            select_probs = select_probs[::-1]#np.array(elite_fitness) / np.sum(elite_fitness)


        offsprings = []
        for i in range(elitism):
            a,b=np.random.choice(range(elitism), 2, p=select_probs)
            ...
            if c2 not in offsprings:
                ...
                offsprings.append(c2)
        # select_probs = np.array(pop_fitness) / np.sum(pop_fitness)
        # offsprings = [crossover(population, pop_fitness, n_chromosomes, select_probs) for _ in range(n_chromosomes-elitism)]
...
if __name__ == '__main__':
    ...
    # env, best_policy, all_results = evolve('CartPole-v0', 20, 50, (1,1,12,5), sid=sid)#123456
